...
<div align="center">
  ...
  <a href="https://hub.docker.com/r/markoshust/magento-php/" target="_blank">
    <img src="https://img.shields.io/docker/pulls/markoshust/magento-php.svg?label=php%20docker%20pulls" alt="Docker Hub Pulls - PHP" />
  </a>
  <a href="https://hub.docker.com/r/markoshust/magento-nginx/" target="_blank">
    <img src="https://img.shields.io/docker/pulls/markoshust/magento-nginx.svg?label=nginx%20docker%20pulls" alt="Docker Hub Pulls - Nginx" />
  </a>
  <a href="https://github.com/nxtmage/docker-magento/graphs/commit-activity" target="_blank">
...
* [markoshust/magento-php (Docker Hub)](https://hub.docker.com/r/markoshust/magento-php/)
  ...
  * [`8.1-fpm`, `8.1-fpm-9`] · [`8.1-fpm-xdebug`, `8.1-fpm-xdebug-9`](images/php/8.1)
  * [`8.2-fpm`, `8.2-fpm-9`] · [`8.2-fpm-xdebug`, `8.2-fpm-xdebug-9`](images/php/8.2)
  * [`8.3-fpm`, `8.3-fpm-7`] · [`8.3-fpm-xdebug`, `8.3-fpm-xdebug-7`](images/php/8.3)
  * [`8.4-fpm`, `8.4-fpm-2`] · [`8.4-fpm-xdebug`, `8.4-fpm-xdebug-2`](images/php/8.4)
  * [`8.5-fpm`, `8.5-fpm-0`] · [`8.5-fpm-xdebug`, `8.5-fpm-xdebug-0`](images/php/8.5)
...
* Tested on macOS and Linux. Windows is supported via **WSL 2** (see the [WSL2 section](#wsl2--windows-setup--fix-for-slow-start--container-failures) below for required fixes).
...
* `3.0.0` specifies the version (for example, Community `2.4.9`).
...
1. In both [`compose.yaml`](https://github.com/nxtmage/docker-magento/blob/master/compose/compose.yaml) and [`compose.healthcheck.yaml`](https://github.com/nxtmage/docker-magento/blob/master/compose/compose.healthcheck.yaml), comment out the OpenSearch container and uncomment the Elasticsearch container.
...
Running Docker under Windows Subsystem for Linux (WSL2) can cause slow container starts, healthcheck timeouts (for example, OpenSearch or MariaDB failing), or random container crashes if not configured properly.

Apply the following fixes.

### 1. Store Files Inside the Linux Filesystem (Critical)

Never place project folders in the Windows filesystem, such as `/mnt/c/Users/...`.

Crossing the 9P filesystem bridge causes severe I/O degradation and can cause containers to fail healthchecks during startup.

**Fix:** Keep your project strictly inside the WSL Linux user home directory, for example:

```text
~/Sites/magento
/home/<username>/projects/magento
```
...
By default, WSL2 can consume all host memory or starve containers of enough RAM during heavy container initialization, resulting in containers exiting or being OOM-killed.
...
> Minimum 6GB memory is recommended; 8GB or more is recommended for Magento/OpenSearch.

Apply changes in PowerShell as Administrator:
...
During cold starts on WSL, container healthchecks for OpenSearch, MariaDB, and RabbitMQ can take longer to become healthy than Docker Compose defaults allow.
...
If a service repeatedly fails its healthcheck during startup, override the `start_period` by creating `compose.override.yaml`:
...
If scripts in `bin/` are saved with Windows CRLF endings, Bash in the containers may throw `\r: command not found` or fail silently on startup.
...
Real-time scanning of the WSL2 virtual disk (`ext4.vhdx`) can significantly degrade I/O performance during container startup.
...
Run this command once in Windows PowerShell as Administrator to ensure incoming Xdebug packets from WSL to the host are not blocked:

```powershell
New-NetFirewallRule `
  -DisplayName "WSL-Xdebug" `
  -Direction Inbound `
...
Review modified files, then restart containers:
...
Files shipped by the template (`compose.yaml`, `compose.dev.yaml`, `compose.healthcheck.yaml`) are overwritten by `bin/update`.

To persist local adjustments, create `compose.override.yaml` in your project root:
...
When you install Magento or Mage-OS via `bin/download` or `onelinesetup`, `bin/detect-versions` runs and pins compatible images for PHP, nginx, OpenSearch, database, RabbitMQ, and cache in the generated `compose.versions.yaml`.
...
| `bin/cliq`                     | Execute command quietly                                   | `bin/cliq bin/magento cache:flush`          |
...
| `bin/mysqldump`                | Export MySQL / MariaDB database dump                      | `bin/mysqldump > magento.sql`               |
...
| `bin/setup-domain`             | Configure domain hostname and base URL                    | `bin/setup-domain magento.test`             |
...
If 2FA is active, retrieve the email verification code via Mailcatcher at:

```text
http://magento.test:1080
```

The code is sent to `john.smith@gmail.com`.

### Caching

Caches automatically refresh on code changes using [mage2tv/magento-cache-clean](https://github.com/mage2tv/magento-cache-clean).

To disable this watcher, comment out the watcher line at the end of `bin/start`.

### Database

The default database is **MariaDB 11.4**.

The internal database hostname within Docker is:

```text
db
```

Connect to the MySQL/MariaDB CLI:
...
3. Sync the file into the container:
...
View emails sent by Magento locally at:
...
Redis is the default cache and session storage engine.

To reconfigure manually on existing instances:
...
PhpMyAdmin is included in `compose.dev.yaml`.
...
Xdebug runs inside a dedicated `phpfpm-xdebug` service.

Requests containing the `XDEBUG_SESSION` cookie are routed to it by Nginx automatically.
...
1. Install the Xdebug Helper browser extension and set the IDE Key to `PHPSTORM`.
...
3. In **Preferences > PHP > Debug**, ensure the debug port is set to `9000,9003`.
4. Create a PHP Remote Debug run configuration listening on the Magento server.

### SSH

To bypass host-mount filesystem latency in IDEs, copy `compose.dev-ssh.yaml` to `compose.dev.yaml` before installation.

Connect via SFTP to localhost on the configured SSH port using:
...
2. For `host.docker.internal` resolution, add the following to `app.extra_hosts` in `compose.yaml`:

```yaml
host.docker.internal:172.17.0.1
```

Match this to your Docker bridge IP:

```bash
docker run --rm alpine ip route | awk 'NR==1 {print $3}'
```

3. For Elasticsearch/OpenSearch, increase the memory map count in `/etc/sysctl.conf`:
...
```bash
bin/setup-ssl store1.example.test store2.example.test
```
...
1. Create a tunnel in Cloudflare Zero Trust and save the token to `env/cloudflare.env`.
2. Uncomment the Cloudflare service in `compose.yaml`.
3. Point the service URL to:

```text
https://<app-container-name>:8443
```
...
Access the SPX profiler UI directly at:

```text
https://magento.test/?SPX_UI_URI=/
```
...
If an installation fails, subsequent attempts may fail because Composer requires an empty directory.
...
Originally created by [Mark Shust](https://github.com/markshust).

Maintained and adapted by [nxtmage/docker-magento](https://github.com/nxtmage/docker-magento).
